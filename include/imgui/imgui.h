...
ImGuiPayload, ImGuiTableSortSpecs, ImGuiTableColumnSortSpecs)
// Obsolete functions
...
IMGUI_API void PushItemWidth(
    float item_width);  // push width of items for common large "item+label"
                        ...
                        // align width to the right side). 0.0f = default to
...
enum ImGuiTableFlags_ {
  ...
  ImGuiTableFlags_NoBordersInBody =
      1 << 11,  // Disable vertical borders in columns Body (borders will always
                // appears in Headers).
  ImGuiTableFlags_NoBordersInBodyUntilResize =
      1 << 12,  // Disable vertical borders in columns Body until hovered for
                // resize (borders will always appears in Headers). Sizing
  ImGuiTableFlags_ColumnsWidthStretch =
      1 << 13,  // Default if ScrollX is off. Columns will default to use
                // _WidthStretch. Read description above for more details.
  ImGuiTableFlags_ColumnsWidthFixed =
      1 << 14,  // Default if ScrollX is on. Columns will default to use
  ...
  ImGuiTableFlags_SameWidths =
      1
      << 15,  // Make all columns the same widths which is useful with Fixed
  ...
  ImGuiTableFlags_NoHeadersWidth =
      1 << 16,  // Disable headers' contribution to automatic width calculation.
  ImGuiTableFlags_NoHostExtendY =
      1 << 17,  // Disable extending past the limit set by outer_size.y, only
...
enum ImGuiTableColumnFlags_ {
  ...
  ImGuiTableColumnFlags_NoHeaderWidth =
      1 << 12,  // Disable header text width contribution to automatic column
                // width.
  ImGuiTableColumnFlags_PreferSortAscending =
      1 << 13,  // Make the initial sort direction Ascending when first sorting
                // on this column (default).
  ImGuiTableColumnFlags_PreferSortDescending =
      1 << 14,  // Make the initial sort direction Descending when first sorting
                // on this column.
...
struct ImFontAtlas {
  ...
  // colorful icons and use them as regular glyphs. Read docs/FONTS.md for more
  // details about using colorful icons. Note: this API may be redesigned later
  // in order to support multi-monitor varying DPI settings.
