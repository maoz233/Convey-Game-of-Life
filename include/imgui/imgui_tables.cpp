...
// widths, columns positions, clipping rectangles. Automatically called by the
...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags,
  ...
  // Adjust flags: disable Resizable when using SameWidths (done above enforcing
  // BordersInnerV)
...
bool ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count,
  ...
  // to facilitate merge draw commands by default. This is because all our cell
  // highlight are manually clipped with BgClipRect Larger at first, if/after
  // unfreezing will become same as tight
...
void ImGui::TableUpdateLayout(ImGuiTable* table) {
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
    ...
    // It would easily work without but we're ready to guarantee it since e.g.
    // names need resubmission anyway. In theory we could be calling
    // TableSetupColumn() here with dummy values it should yield the same
    // effect.
    ...
    if (column->AutoFitQueue != 0x00) want_column_auto_fit = true;
  ...
  // wait until the first visible frame of the child container (may or not be a
  // good thing).
  ...
  if (want_column_auto_fit) table->IsSettingsDirty = true;
  ...
  const float min_column_distance = min_column_width +
  ...
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
    ...
    if (column->Flags & (ImGuiTableColumnFlags_WidthFixed |
      ...
      // Revert or initialize weight (when column->StretchWeight < 0.0f normally
      // it means there has been no init value so it'll always default to 1.0f)
      if (column->AutoFitQueue != 0x00 || column->StretchWeight < 0.0f)
        column->StretchWeight = (column->InitStretchWeightOrWidth > 0.0f)
  ...
  // - When all columns are fixed or columns are of mixed type: use the maximum
  // auto width.
  ...
  if (table->Flags & ImGuiTableFlags_SameWidths) {
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
      ...
      if (column->Flags & (ImGuiTableColumnFlags_WidthFixed |
        ...
        if (mixed_same_widths) column->WidthRequest = max_width_auto;
      }
    }
  }
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX) {
      ...
      // Because of this we also know that all of the columns will always fit in
      // table->WorkRect and therefore in table->InnerRect (because ScrollX is
      // off)
...
void ImGui::EndTable() {
  ...
  if (inner_window != outer_window) {
    table->OuterRect.Max.y = ImMax(table->OuterRect.Max.y,
  ...
  // Override EndChild/ItemSize max extent with our own to enable auto-resize on
  // the X axis when possible
  // FIXME-TABLE: This can be improved (e.g. for Fixed columns we don't want to
  // auto AutoFitWidth? or propagate window auto-fit to table?)
  if (table->Flags & ImGuiTableFlags_ScrollX) {
...
void ImGui::TableSetColumnWidth(int column_n, float width) {
  ...
  // will not move. (forwarded by Resize Rule 2)
  ...
  // - W1 F2 W3  resize W1| or F2|        --> symmetrical resize is weird and
  // glitchy. Seems possible to fix.
  ...
  if (column_0->Flags & ImGuiTableColumnFlags_WidthFixed) {
    // [Resize Rule 3] If we are are followed by a fixed column and we have a
    // Stretch column before, we need to ensure that our left border won't move,
    // which we can do by making sure column_a/column_b resizes cancels each
    // others.
    ...
    if (column_1->Flags & ImGuiTableColumnFlags_WidthFixed) {
      ...
      // At this point column_1 is the next OR previous column and we know it is
      // a stretch column. (old_a + old_b == new_a + new_b) --> (new_a == old_a
      // + old_b - new_b)
      float column_1_width = ImMax(
          column_1->WidthRequest - (column_0_width - column_0->WidthRequest),
          min_width);
...
void ImGui::TableSetColumnWidthAutoAll(ImGuiTable* table) {
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
    ...
    if (!column->IsEnabled &&
        !(column->Flags &
          ImGuiTableColumnFlags_WidthStretch))  // Can reset weight of hidden
                                                // stretch column
      continue;
    column->CannotSkipItemsQueue = (1 << 0);
    column->AutoFitQueue = (1 << 1);
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table) {
  ...
  if (merge_group_mask != 0) {
    // We skip channel 0 (Bg0) and 1 (Bg1 frozen) from the shuffling since they
    ...
    for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups);
      ...
      // Make sure Bg1DrawChannelUnfrozen appears in the middle of our groups
      // (whereas Bg0 and Bg1 frozen are fixed to 0 and 1)
      if (merge_group_n == 1 && has_freeze_v)
        memcpy(dst_tmp++, &splitter->_Channels[table->Bg1DrawChannelUnfrozen],
...
void ImGui::TableDrawBorders(ImGuiTable* table) {
  ...
  if (table->Flags & ImGuiTableFlags_BordersInnerV) {
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
      ...
      if (column->NextEnabledColumn == -1 && !is_resizable)
        if ((table->Flags & ImGuiTableFlags_SameWidths) == 0) continue;
      if (column->MaxX <=
          column->ClipRect.Min
  ...
  if (table->Flags & ImGuiTableFlags_BordersOuter) {
    ...
    if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
      ...
      outer_drawlist->AddLine(outer_border.Min,
                              ImVec2(outer_border.Min.x, outer_border.Max.y),
                              outer_col, border_size);
      outer_drawlist->AddLine(ImVec2(outer_border.Max.x, outer_border.Min.y),
                              outer_border.Max, outer_col, border_size);
    } else if (table->Flags & ImGuiTableFlags_BordersOuterH) {
      outer_drawlist->AddLine(outer_border.Min,
                              ImVec2(outer_border.Max.x, outer_border.Min.y),
                              outer_col, border_size);
      outer_drawlist->AddLine(ImVec2(outer_border.Min.x, outer_border.Max.y),
                              outer_border.Max, outer_col, border_size);
    }
  }
  if ((table->Flags & ImGuiTableFlags_BordersInnerH) &&
...
void ImGui::TableHeader(const char* label) {
  ...
  if (hovered || selected) {
    const ImU32 col =
        GetColorU32(held ? ImGuiCol_HeaderActive
...
void ImGui::DebugNodeTable(ImGuiTable* table) {
  ...
  for (int n = 0; n < table->ColumnsCount; n++) {
    ...
    ImFormatString(
        ...
        (column->StretchWeight / sum_weights) * 100.0f, column->MinX,
